...
require 'Segment.php';
class WPES_OdfException extends Exception
{}
/**
...
class WPES_Odf
{
     ...
     * @throws WPES_OdfException
     */
    public function __construct($filename, $config = array())
    {
    	if (! is_array($config)) {
    		throw new WPES_OdfException('Configuration data must be provided as array');
    	}
        ...
        if (! class_exists($this->config['ZIP_PROXY'])) {
            throw new WPES_OdfException($this->config['ZIP_PROXY'] . ' class not found - check your php settings');
        }
        $zipHandler = $this->config['ZIP_PROXY'];
        $this->file = new $zipHandler();
        if ($this->file->open($filename) !== true) {
            throw new WPES_OdfException("Error while Opening the file '$filename' - Check your odt file");
        }
        if (($this->contentXml = $this->file->getFromName('content.xml')) === false) {
            throw new WPES_OdfException("Nothing to parse - check that the content.xml file is correctly formed");
        }
		if (($this->stylesXml = $this->file->getFromName('styles.xml')) === false) {
			throw new WPES_OdfException("Nothing to parse - Check that the styles.xml file is correctly formed in source file '$filename'");
		}
		if (($this->manifestXml = $this->file->getFromName('META-INF/manifest.xml')) === false) {
			throw new WPES_OdfException("Something is wrong with META-INF/manifest.xm in source file '$filename'");
 			}
    ...
    public function setVars($key, $value, $encode = true, $charset = 'ISO-8859')
    {
		...
		if (strpos($this->contentXml, $tag) === false && strpos($this->stylesXml , $tag) === false) {
 				throw new WPES_OdfException("var $key not found in the document");
 			}
    ...
    public function setImage($key, $value)
    {
        ...
        if ($size === false) {
            throw new WPES_OdfException("Invalid image");
        }
...
IMG;
    ...
    public function mergeSegment(Segment $segment)
    {
        if (! array_key_exists($segment->getName(), $this->segments)) {
            throw new WPES_OdfException($segment->getName() . 'cannot be parsed, has it been set yet ?');
        }
    ...
    public function setSegment($segment)
    {
        ...
        if (preg_match($reg, html_entity_decode($this->contentXml), $m) == 0) {
            throw new WPES_OdfException("'$segment' segment not found in the document");
        }
    ...
    public function saveToDisk($file = null)
    {
        if ($file !== null && is_string($file)) {
        	if (file_exists($file) && !(is_file($file) && is_writable($file))) {
            	throw new WPES_OdfException('Permission denied : can\'t create ' . $file);
        	}
            $this->_save();
    ...
    private function _save()
    {
        ...
        if (! $this->file->addFromString('content.xml', $this->contentXml) || ! $this->file->addFromString('styles.xml' , $this->stylesXml ) ) {
 			throw new WPES_OdfException('Error during file export addFromString');
        }
       ...
       //$this->manifestXml = $this->manifestXml ."\n".$manifdata;
        ...
        if (! $this->file->addFromString('META-INF/manifest.xml', $this->manifestXml)) {
            throw new WPES_OdfException('Error during manifest file export');
        }
    ...
    public function exportAsAttachedFile($name="")
    {
        ...
        if (headers_sent($filename, $linenum)) {
            throw new WPES_OdfException("headers already sent ($filename at $linenum)");
        }
